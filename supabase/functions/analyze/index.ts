...
// Location: supabase/functions/analyze/index.ts
//
// Description:
...
// 3. Forwards the user's prompt to the modern Gemini 3.5 Flash model.
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method !== "POST") {
      return new Response(
        JSON.stringify({
          ...
          details: "This edge function exclusively accepts POST requests containing prompt payloads."
        }),
        { 
          status: 405, 
    ...
    // 3. Read GEMINI_API_KEY from Supabase secrets
    ...
    const { prompt, model = "gemini-3.5-flash", temperature = 0.7, systemInstruction } = body;
    ...
    // Use Gemini Developer API endpoint (v1beta API key credential)
